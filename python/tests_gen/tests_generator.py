...
def generate_test_results(seed: int = 1) -> dict[str, Any]:
    rng: np.random.Generator = np.random.default_rng(seed)
    shape: tuple[int, int] = (10, 10)
    s: np.ndarray = rng.uniform(SETTINGS.SPOT_MIN, SETTINGS.SPOT_MAX, shape)
    k: np.ndarray = rng.uniform(SETTINGS.SPOT_MIN, SETTINGS.SPOT_MAX, shape)
...
if __name__ == "__main__":
    results: dict[str, Any] = generate_test_results()
