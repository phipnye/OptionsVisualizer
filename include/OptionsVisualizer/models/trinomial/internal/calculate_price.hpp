...
Eigen::MatrixXd calculatePrice(const Eigen::DenseIndex nSigma,
                               const Eigen::DenseIndex nStrike,
                               const double spot, const double r,
                               const double q, const auto& sigmasGrid,
                               const Eigen::MatrixXd& strikesGrid,
                               const double tau) {
  static_assert(
  ...
  const Eigen::VectorXd u{
      (sigmasGrid.matrix().col(0).array() * std::sqrt(3.0 * dTau))
          .exp()
          .eval()};
  ...
  // Use auto for expression templates to avoid temporary matrix allocations for
  // intermediate steps
  ...
  // Pre-allocate buffers and exercise storage (avoids heap allocations in the
  // loop)
  constexpr Eigen::DenseIndex maxNodes{2 * trinomialDepth + 1};
  std::vector<Eigen::MatrixXd> nextOptionValues(
      maxNodes, Eigen::MatrixXd(nSigma, nStrike));
  std::vector<Eigen::MatrixXd> currOptionValues(
      maxNodes, Eigen::MatrixXd(nSigma, nStrike));

  // Calculate option values at expiration
  const Eigen::MatrixXd expirationSpot{helpers::buildSpotLattice(
      spot, u, trinomialDepth, nSigma)};  // shape: [node, sigma]

  // Calculate payoff at expiration (intrinsic value only at expriation)
  for (Eigen::DenseIndex node{0}; node < maxNodes; ++node) {
    nextOptionValues[node] = helpers::intrinsicValue<T>(
  ...
  for (Eigen::DenseIndex depth{trinomialDepth - 1}; depth > -1; --depth) {
    ...
    const Eigen::MatrixXd spotsDepth{
        helpers::buildSpotLattice(spot, u, depth, nSigma)};
    ...
    for (Eigen::DenseIndex node{0}; node < nNodes; ++node) {
      ...
      // from next depth
      const Eigen::MatrixXd& valUp{
          nextOptionValues[static_cast<std::size_t>(node + 2)]};
      const Eigen::MatrixXd& valMid{
          nextOptionValues[static_cast<std::size_t>(node + 1)]};
      const Eigen::MatrixXd& valDown{
          nextOptionValues[static_cast<std::size_t>(node)]};

      // Calculate expected value: (pU * valUp + pM * valMid + pD * valDown) *
      // discount
      const Eigen::MatrixXd continuationValue{(pU.array() * valUp.array() +
                                               pM.array() * valMid.array() +
                                               pD.array() * valDown.array()) *
                                              discountFactor};

      // Update optionValues: American early exercise check
      currOptionValues[node] =
          continuationValue.cwiseMax(helpers::intrinsicValue<T>(
