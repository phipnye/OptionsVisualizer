...
public:
    PricingSurface(Eigen::DenseIndex nSigma, Eigen::DenseIndex nStrike, double spot, double r, double q, double sigmaLo,
                   double sigmaHi, double strikeLo, double strikeHi, double tau, BS::thread_pool<>& pool);
...
private:
    ...
    GreeksResult trinomialGreeks() const {
        ...
        for (const Perturb& p : perturbations) {
            futures.emplace_back(pool_.submit_task([p, this]() {
                ...
                return models::trinomial::calculatePrice<T>(this->nSigma_, this->nStrike_,
        ...
        for (std::size_t perturbIdx{0}; perturbIdx < perturbations.size(); ++perturbIdx) {
            prices[Perturb::getIdx(perturbations[perturbIdx].idx)] = futures[perturbIdx].get();
        }
        ...
        Eigen::MatrixXd delta{firstOrderCDM(prices[Perturb::getIdx(Perturb::Idx::LoSpot)],
        ...
        Eigen::MatrixXd vega{firstOrderCDM(prices[Perturb::getIdx(Perturb::Idx::LoSigma)],
        ...
        Eigen::MatrixXd theta{-firstOrderCDM(prices[Perturb::getIdx(Perturb::Idx::LoTau)],
                                             prices[Perturb::getIdx(Perturb::Idx::HiTau)], hTau)};

        // Compute rho (first derivate w.r.t r) using CDM
        Eigen::MatrixXd rho{firstOrderCDM(prices[Perturb::getIdx(Perturb::Idx::LoRho)],
