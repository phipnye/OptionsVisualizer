...
public:
    ...
    [[nodiscard]] std::array<Eigen::MatrixXd, globals::nGrids> calculateGrids() const;

private:
    // --- Black-Scholes-Merton
    [[nodiscard]] GreeksResult bsmCallGreeks() const;

    [[nodiscard]] GreeksResult bsmPutGreeks(const GreeksResult &callResults) const;
    ...
    [[nodiscard]] GreeksResult trinomialGreeks() const {
        ...
        const auto firstOrderCDM{
            [](const Eigen::MatrixXd &lo, const Eigen::MatrixXd &hi, const double eps) {
        ...
        Eigen::MatrixXd gamma{
            (prices[Perturb::getIdx(Perturb::Idx::HiSpot)] -
        ...
        return GreeksResult{
            std::move(prices[Perturb::getIdx(Perturb::Idx::Base)]),
            std::move(delta),
            std::move(gamma),
            std::move(vega),
            std::move(theta),
