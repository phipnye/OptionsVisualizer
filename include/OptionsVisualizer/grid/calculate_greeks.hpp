...
template <typename T>
py::array_t<T> calculateGreeksGrid(T spot_, py::array_t<T, py::array::c_style> strikes_, T r_, T q_,
    ...
    torch::Tensor sigmas{
        torch::from_blob(sigmas_.mutable_data(), {nSigma}, torch::dtype(torch::CppTypeToScalarType<T>::value))};
    torch::Tensor tau{torch::tensor(tau_)};

    // Release gill (no longer need access to python objects)
    py::gil_scoped_release noGil{};

    greeks::GreeksResult amerCall{greeks::trinomialCallGreeks(spot, strikes, r, q, sigmas, tau)};
    greeks::GreeksResult amerPut{greeks::trinomialPutGreeks(spot, strikes, r, q, sigmas, tau)};
    greeks::GreeksResult euroCall{greeks::bsmCallGreeks(spot, strikes, r, q, sigmas, tau)};
    greeks::GreeksResult euroPut{greeks::bsmPutGreeks(spot, strikes, r, q, sigmas, tau)};
    ...
    py::array::ShapeContainer shape{static_cast<py::ssize_t>(nSigma), static_cast<py::ssize_t>(nStrike), 4L, 5L};
